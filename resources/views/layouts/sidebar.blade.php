...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-radius-lg fixed-end me-2 rotate-caret bg-white my-2" id="sidenav-main">
  ...
  <div class="collapse navbar-collapse px-0 w-auto" id="sidenav-collapse-main">
      <ul class="navbar-nav">
          ...
          <!-- All Categories Link -->
          <li class="nav-item">
              <a class="nav-link text-dark {{ is_null($currentCategoryId) ? 'active' : '' }}" href="{{ route('home') }}">
          ...
          @if(isset($navcategories) && $navcategories->count() > 0)
              @foreach ($navcategories as $category)
                  <li class="nav-item">
                      <a 
                          class="nav-link text-dark {{ $currentCategoryId == $category->id ? 'bg-gradient-dark text-white' : '' }}" 
          ...
          @else
              <li class="nav-item">
                  <span class="nav-link text-dark">No categories available</span>
              </li>
          @endif
      </ul>
  </div>
